...
from Gaugi import calcSP, Logger, mkdir_p, progressbar
from Gaugi.messenger.macros import *
from Gaugi import retrieve_kw,checkForUnusedVars, NotSet, LoggingLevel
from monet import *
import numpy as np
import os


class TableBoard( Logger ):
  ...
  def __init__(self, cvfile, rootfile, **kw):
    ...
    try:#Protection
      MSG_INFO(self, 'Reading root file (%s)...',rootfile)
      self._rootObj = TFile(rootfile, 'read')
    except RuntimeError:
      MSG_FATAL(self, 'Could not open root monitoring file.')
    from Gaugi import load
    try:#Protection
      MSG_INFO(self, 'Reading summary file (%s)...', cvfile)
      cvObj = load(cvfile)
    except RuntimeError:
      MSG_FATAL(self, 'Could not open pickle summary file.')
    #Loop over benchmarks

    from tableboard.utilities import Summary
    for benchmark in cvObj.keys():
      ...
      csummary = Summary( benchmark, cvObj[benchmark] )
      self._summaryObjs.append( csummary ) 
      MSG_INFO(self, 'Creating Summary for %s in [et=%d, eta=%d]', benchmark,csummary.etBinIdx(), csummary.etaBinIdx())
  ...
  def __call__(self, **kw):
    ...
    for csummary in self._summaryObjs:
      ...
      if csummary.etaBinIdx() != self._etaBinIdx or csummary.etBinIdx() != self._etBinIdx:
        MSG_FATAL(self, "Benchmark dictionary is not compatible with the et/eta Indexs")
    ...
    self.cvReport( csummary, title, outname, toPDF=toPDF, doOnlyTables=doOnlyTables )
    return ret
  #End of loop()
  ...
  def cvReport( self, csummary, title, outname, toPDF=True, doOnlyTables=False):
